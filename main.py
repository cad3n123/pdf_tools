...
# ================= CONFIGURATION =================
# Where users will look for updates (Your Repo)
GITHUB_RAW_URL = "https://raw.githubusercontent.com/cad3n123/pdf_tools/main/MasterPDFTool_Offline.html"
VERSION = "1.0"
OUTPUT_FILE = "MasterPDFTool_Offline.html"
...
def main():
    print("==== Master PDF Tool Builder ====")
    
    # 1. Download Libraries
    lib_content = {}
    try:
        for name, url in LIBS.items():
            lib_content[name] = fetch_url(url).decode('utf-8')
    except Exception as e:
        print(f"Error downloading libraries: {e}")
        return

    # 2. Encode Worker (JS to Base64) to avoid escaping issues
    print("Encoding Worker logic...")
    worker_bytes = lib_content['pdfworker'].encode('utf-8')
    worker_b64 = base64.b64encode(worker_bytes).decode('utf-8')

    # 3. The Application Code (CSS/HTML/JS)
    # We bake this into a variable so the "Smart Loader" can inject it
    app_css = """
        :root { --primary: #2563eb; --bg: #f8fafc; --card: #ffffff; }
        body { overflow: auto; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background: var(--bg); display: flex; justify-content: center; min-height: 100vh; margin: 0; padding: 20px; color: #333; }
        ...
        .update-bar { background: #dcfce7; color: #166534; padding: 10px; text-align: center; font-size: 0.9em; cursor: pointer; display:none; border-bottom: 1px solid #bbf7d0; }
    """

    app_html = f"""
        <div class="container">
            <div id="update-notification" class="update-bar" onclick="location.reload()">
                Update available! Using new version automatically. (Click to reload)
            </div>
            <div class="tabs">
            ...
            <div style="text-align:center; margin-top:10px; font-size:0.8em; color:#999;">v{VERSION}</div>
        </div>
    """

    # 4. Construct the Final HTML File
    # The "Smart Loader" logic is the <script> block at the bottom
...
<head>
    ...
    <script>
        ...
        const UPDATE_URL = "{GITHUB_RAW_URL}";
        const WORKER_B64 = "{worker_b64}";
        
        // Libraries embedded as strings
        const LIB_PDFLIB = {json.dumps(lib_content['pdf-lib'])};
        const LIB_DL = {json.dumps(lib_content['downloadjs'])};
        const LIB_ZIP = {json.dumps(lib_content['jszip'])};
        const LIB_PDFJS = {json.dumps(lib_content['pdfjs'])};
        
        const APP_CSS = {json.dumps(app_css)};
        const APP_HTML = {json.dumps(app_html)};
    </script>
</head>
<body>
    <div id="loader-ui">
    ...
    (async function() {{
            ...
            try {{
                ...
                if (response.ok) {{
                    ...
                    if (remoteVer > CURRENT_VERSION) {{
                        txt.innerText = "New version (" + remoteVer + ") found! Loading...";
                        bar.style.width = "90%";
                        
                        // HOT SWAP: Replace entire document with new version
                        document.open();
                ...
                console.log("Offline or update check failed:", e);
            }}
        }}

        // --- NO UPDATE FOUND: LAUNCH LOCAL ---
        launchApp("Launching...");
        ...
        function injectApp() {{
            ...
            // 5. Inject Application Logic (Functions)
            ...
            window.updateLabel = function(id, lbl) {{ 
                const i = document.getElementById(id); 
                const c = i.files.length; 
                document.getElementById(lbl).innerText = c > 0 ? c + " file(s)" : "Click to select"; 
            }};
            
            window.setStatus = function(id, m, t) {{ 
            ...
            window.mergePDFs = async function() {{
                ...
                if (!files.length) return setStatus('mergeStatus', 'Select files.', 'error');
