...
REM !!! CONFIGURATION: REPLACE THIS URL !!!
...
echo Write-Host "2. Downloading libraries..."
echo $libs = @{
echo     "pdflib" = (Invoke-WebRequest "https://unpkg.com/pdf-lib@1.17.1/dist/pdf-lib.min.js").Content;
echo     "downloadjs" = (Invoke-WebRequest "https://unpkg.com/downloadjs@1.4.7/download.min.js").Content;
echo     "jszip" = (Invoke-WebRequest "https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js").Content;
echo     "pdfjs" = (Invoke-WebRequest "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.min.js").Content;
echo     "worker" = (Invoke-WebRequest "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.worker.min.js").Content
echo }

echo Write-Host "3. Encoding Worker..."
echo $workerB64 = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($libs["worker"]))

echo Write-Host "4. Creating Smart HTML..."
echo $html = @"
echo ^<!DOCTYPE html^>
echo ^<html lang="en"^>
echo ^<head^>
...
echo     ^<title^>Master PDF Tool (Smart)^</title^>
echo     ^<style^>
echo         body { font-family: sans-serif; margin: 0; overflow: hidden; background: #f8fafc; }
echo         #loader-ui { position: fixed; top: 0; left: 0; width: 100%%; height: 100%%; background: #fff; z-index: 9999; display: flex; flex-direction: column; justify-content: center; align-items: center; transition: opacity 0.5s; }
echo         .spinner { width: 50px; height: 50px; border: 5px solid #e2e8f0; border-top: 5px solid #2563eb; border-radius: 50%%; animation: spin 1s linear infinite; margin-bottom: 20px; }
echo         @keyframes spin { 0%% { transform: rotate(0deg); } 100%% { transform: rotate(360deg); } }
echo         #status-text { font-size: 1.2rem; color: #333; font-weight: 500; }
echo         #progress-bar { width: 300px; height: 6px; background: #e2e8f0; border-radius: 3px; margin-top: 15px; overflow: hidden; }
echo         #progress-fill { height: 100%%; width: 0%%; background: #2563eb; transition: width 0.3s; }
echo     ^</style^>
echo     ^<script^>
echo         const CURRENT_VERSION = "$ver";
echo         const UPDATE_URL = "$url";
echo         // --- APP LIBRARIES ---
echo         const LIB_PDFLIB = \`$($libs["pdflib"].Replace('`', '\`').Replace('$', '\$'))\`;
echo         const LIB_DL = \`$($libs["downloadjs"].Replace('`', '\`').Replace('$', '\$'))\`;
echo         const LIB_ZIP = \`$($libs["jszip"].Replace('`', '\`').Replace('$', '\$'))\`;
echo         const LIB_PDFJS = \`$($libs["pdfjs"].Replace('`', '\`').Replace('$', '\$'))\`;
echo         const WORKER_B64 = "$workerB64";
echo     ^</script^>
echo ^</head^>
echo ^<body^>
echo     ^<div id="loader-ui"^>
echo         ^<div class="spinner"^>^</div^>
echo         ^<div id="status-text"^>Checking for updates...^</div^>
echo         ^<div id="progress-bar"^>^<div id="progress-fill"^>^</div^>^</div^>
echo     ^</div^>
echo     ^<script^>
echo     (async function() {
echo         const ui = document.getElementById('loader-ui');
echo         const txt = document.getElementById('status-text');
echo         const bar = document.getElementById('progress-fill');
echo         
echo         function launchApp(isUpdate) {
echo             txt.innerText = isUpdate ? "Update found! Launching..." : "Offline mode. Launching...";
echo             bar.style.width = "100%%";
echo             setTimeout(() =^> {
echo                 ui.style.opacity = '0';
echo                 setTimeout(() =^> ui.remove(), 500);
echo                 injectApp();
echo             }, 500);
echo         }
echo         
echo         // 1. Try to fetch update
echo         if (!UPDATE_URL.includes("YOUR_USERNAME")) {
echo             try {
echo                 const controller = new AbortController();
echo                 const timeoutId = setTimeout(() =^> controller.abort(), 5000); // 5s timeout
echo                 
echo                 txt.innerText = "Checking repository...";
echo                 bar.style.width = "30%%";
echo                 
echo                 const response = await fetch(UPDATE_URL + '?t=' + Date.now(), { signal: controller.signal });
echo                 clearTimeout(timeoutId);
echo                 
echo                 if (response.ok) {
echo                     const remoteHtml = await response.text();
echo                     // Extract version from remote HTML
echo                     const match = remoteHtml.match(/const CURRENT_VERSION = "([^"]+)"/);
echo                     const remoteVer = match ? match[1] : "0.0";
echo                     
echo                     if (remoteVer ^> CURRENT_VERSION) {
echo                         txt.innerText = "New version (" + remoteVer + ") found! Downloading...";
echo                         bar.style.width = "80%%";
echo                         // HOT SWAP: Overwrite current document with new one
echo                         document.open();
echo                         document.write(remoteHtml);
echo                         document.close();
echo                         return; // Stop execution here
echo                     }
echo                 }
echo             } catch (e) { console.log("Offline or check failed", e); }
echo         }
echo         // 2. Fallback to embedded
echo         launchApp(false);
echo         
echo         function injectApp() {
echo             // Inject CSS
...
echo             
echo             // Inject Libs
echo             const s1 = document.createElement('script'); s1.textContent = LIB_PDFLIB; document.head.appendChild(s1);
echo             const s2 = document.createElement('script'); s2.textContent = LIB_DL; document.head.appendChild(s2);
echo             const s3 = document.createElement('script'); s3.textContent = LIB_ZIP; document.head.appendChild(s3);
echo             const s4 = document.createElement('script'); s4.textContent = LIB_PDFJS; document.head.appendChild(s4);
echo             
echo             // Setup Worker
echo             const workerBlob = new Blob([atob(WORKER_B64)], { type: 'text/javascript' });
echo             pdfjsLib.GlobalWorkerOptions.workerSrc = URL.createObjectURL(workerBlob);
echo             
echo             // Inject UI
...
echo             
echo             // Inject Logic
echo             window.switchTab = function(id) { document.querySelectorAll('.content').forEach(el =^> el.classList.remove('active')); document.querySelectorAll('.tab').forEach(el =^> el.classList.remove('active')); document.getElementById(id).classList.add('active'); event.target.classList.add('active'); clearStatus(); };
echo             window.updateLabel = function(id, lbl) { const i = document.getElementById(id); const c = i.files.length; document.getElementById(lbl).innerText = c ^> 0 ? c + " file(s)" : "Click to select"; };
echo             window.setStatus = function(id, m, t) { const el = document.getElementById(id); el.innerText = m; el.className = 'status ' + t; };
echo             window.clearStatus = function() { document.querySelectorAll('.status').forEach(el =^> el.innerText = ''); };
echo             window.readFileAsArrayBuffer = async function(file) { return new Promise((resolve, reject) =^> { const r = new FileReader(); r.onload = () =^> resolve(r.result); r.onerror = reject; r.readAsArrayBuffer(file); }); };
echo             
...
echo ^</body^>
echo ^</html^>
echo "@ | Out-File -Encoding utf8 "%OUTFILE%"
) > "%PWS%"
